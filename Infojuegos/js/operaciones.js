...
$(document).ready(function(){
    $('#registrar').click(function(){
        ...
        var ccontra = $('#ccontra').val();
        var correo = $('#correo').val();
        var pais = $('#pais').val();
        if (usuario != '' && contra != '' && ccontra != '' && correo != '' && pais != '' ){
            $.ajax({
                url: '../model/crear.php',
                method: 'POST',
                data: {usuario: usuario, contra: contra, ccontra: ccontra, correo: correo, pais: pais},
                success: function(msg){
                  if (msg=='1') {
                    $('#alerta').html('el usuario existe');

                  }else if (msg=='2') {
                     $('#alerta').html('las claves ingresadas no considen');
